...
<html lang="en">
  ...
  <!--<![endif]-->
    ...
    <div class="sidebar-nav">
        ...
        <ul id="${item.key}" class="nav nav-list collapse<c:if test="${item.name=='客房管理'}"> in</c:if>">
        <c:forEach items="${permissionlist}" var="itemchil">
        <c:if test="${item.id==itemchil.par_id }">
            <li <c:if test="${itemchil.name=='客房设置'}">class='active'</c:if>>
    ...
    <div class="content">
        ...
        <div class="container-fluid">
            <div class="row-fluid">
<form action="<c:url value='/roomEdit'/>" method="post">  
${msg}       
...
<div class="well">
		<label>客房id</label>
		<input type="text" name="id" value="${room.id }" class="input-xlarge" readonly="readonly">
        <label>客房类型</label>
        <select class="input-xlarge" id="selecttype" name="type">
        <option value="单人间">单人间</option>
        <option value="双人间">双人间</option>
        <option value="豪华间">豪华间</option>
        </select>
        <label>客房价格</label>
        <input type="text" name="price" value="${room.price }" class="input-xlarge">
        <label>可住人数</label>
        <input type="text" name="number" value="${room.number }" class="input-xlarge">
        <label>客房状态</label>
    ...
    <!-- 控制下拉框默认值 -->
    <script type="text/javascript">
    document.getElementById("selecttype").value="${room.type}";
