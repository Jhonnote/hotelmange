...
<html lang="en">
  ...
  <!--<![endif]-->
    ...
    <div class="sidebar-nav">
        ...
        <ul id="${item.key}" class="nav nav-list collapse in">
        <c:forEach items="${permissionlist}" var="itemchil">
        <c:if test="${item.id==itemchil.par_id }">
            <li <c:if test="${itemchil.name=='角色管理'}">class='active'</c:if>>
    ...
    <div class="content">
        ...
        <ul class="breadcrumb">
            ...
            <li><a href="users.html">角色管理</a> <span class="divider">/</span></li>
            <li class="active">角色管理</li>
        </ul>
        <div class="container-fluid">
            <div class="row-fluid">
<form action="<c:url value='/rolebind'/>" method="post">  
...
<div class="well">
<label>${msg}</label>
        <span><label>角色名称:${role.name}</label></span>
        
        <label>状态:<c:choose>
        <c:when test="${role.status==0 }">可用</c:when>
        <c:otherwise>禁用</c:otherwise>
        </c:choose></label>
        <c:forEach items="${mypermission}" var="items">
        <label><input type="checkbox" checked="checked" name="permission" value="${items.id}"/>${items.name }</label>
        </c:forEach>
        <c:forEach items="${allpermissionList}" var="item">
        <label><input style="margin:0;" type="checkbox" name="permission" value="${item.id}"/>${item.name }</label>
        </c:forEach>
</div>
